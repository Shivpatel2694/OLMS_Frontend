...
const ManagerList = () => {
  ...
  const departments = ["Human Resources","Engineering","Finance","Marketing","Sales","Customer Support","IT & Infrastructure","Research & Development","Other Departments"]
...
//   ];
  ...
  useEffect(() => {
    ...
    const fetchManagers = async () => {
      ...
      try {
        ...
        // Add a slight delay to simulate network loading
        setTimeout(() => {
          setIsLoading(false);
        }, 1000);
  ...
  // Function to search managers by email
const searchManagersByEmail = async (email) => {
    ...
    try {
      if (!email) {
        ...
        const response = await axios.get(`http://localhost:8080/api/admin/managers/search-by-email`, {
          params: { email },
          headers: {
            Authorization: `Bearer ${token}`
          }
  ...
  const handleCreateManager = async () => {
    if (!newManager.firstName || !newManager.lastName || !newManager.email || !newManager.department) {
    ...
    try {
      ...
      /*
      const response = await axios.post('http://localhost:8080/api/admin/managers', newManager, {
        headers: {
          Authorization: `Bearer ${token}`,
      ...
      // Add the newly created manager to the list with full name
      ...
      */
      ...
      // For visualization, create a new manager with the form data
      const createdManager = {
        ...newManager,
  ...
  const confirmDeleteManagers = async () => {
    try {
      ...
      for (const managerId of selectedManagers) {
        await axios.delete(`http://localhost:8080/api/admin/managers/${managerId}`, {
          headers: {
            Authorization: `Bearer ${token}`
          }
        });
      ...
      <Layout>
        <div className="min-h-screen bg-gray-100 flex items-center justify-center" style={{ backgroundColor: colorPalette.background }}>
          <div className="text-center">
            ...
            <p className="mt-4" style={{ color: colorPalette.dark }}>Loading manager data...</p>
          </div>
        </div>
      </Layout>
    );
      ...
      <div className="min-h-screen py-6" style={{ backgroundColor: colorPalette.background }}>
        <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex justify-between items-center mb-6">
            <h1 className="text-2xl font-semibold" style={{ color: colorPalette.dark }}>Manager List</h1>
          ...
          <div className="mb-6">
            <div className="relative rounded-md shadow-sm">
              <div className="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none">
                <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" style={{ color: colorPalette.light }}>
          ...
          <div className="bg-white rounded-lg shadow-md overflow-hidden" style={{ borderRadius: "16px" }}>
            <div className="border-b border-gray-200 p-4" style={{ backgroundColor: "#F8F8FB", borderColor: "#E8E8EF" }}>
              <h2 className="text-lg font-medium" style={{ color: colorPalette.dark }}>Company Managers</h2>
            </div>
            ...
            {managers?.length === 0 ? (
              ...
              <div className="p-4">
                {Object.keys(groupedManagers).length > 0 ? (
                  Object.entries(groupedManagers).map(([department, departmentManagers]) => (
                    <div key={department} className="mb-6">
                      <h3 className="text-md font-medium mb-3 ml-1" style={{ color: colorPalette.medium }}>{department} Department</h3>
                      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
                        {departmentManagers.map(manager => (
                          <EmployeeListItem 
                            ...
                            onSelect={() => toggleManagerSelection(manager.id)}
                          />
                        ))}
                      </div>
                    </div>
              ...
              <div className="px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
                ...
                <div className="mt-4 space-y-4">
                  <div className="grid grid-cols-2 gap-4">
                    <div>
                      <label htmlFor="firstName" className="block text-sm font-medium" style={{ color: colorPalette.dark }}>
                        First Name *
                      </label>
                      <input
                        type="text"
                    ...
                    <div>
                      <label htmlFor="lastName" className="block text-sm font-medium" style={{ color: colorPalette.dark }}>
                        Last Name *
                      </label>
                      <input
                        type="text"
                  ...
                  <div>
                    <label htmlFor="email" className="block text-sm font-medium" style={{ color: colorPalette.dark }}>
                      Email Address *
                    </label>
                    <input
                      type="email"
                  ...
                  <div>
                    <label htmlFor="department" className="block text-sm font-medium" style={{ color: colorPalette.dark }}>
                      Department *
                    </label>
                    <input
                      type="text"
                  ...
                  <div>
                    <label htmlFor="role" className="block text-sm font-medium" style={{ color: colorPalette.dark }}>
                      Role
                    </label>
                    <select
                      id="role"
                  ...
                  <div className="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left">
                    ...
                    <div className="mt-2">
                      <p style={{ color: colorPalette.medium }}>
                        Are you sure you want to delete {selectedManagers.length} selected manager{selectedManagers.length > 1 ? 's' : ''}? This action cannot be undone.
                      </p>
                    </div>
                  </div>
                </div>
